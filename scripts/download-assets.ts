...
// Configuration
const assetPattern = /(https?:\/\/.*?madeirafriends.*?\.(?:jpg|jpeg|png|gif|svg|webp))/gi
  ...
  try {
    ...
    for (const file of files) {
      ...
      for (const url of urls) {
        ...
        if (assetsMap[url]) {
          content = content.replace(new RegExp(url.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), assetsMap[url])
          modified = true
          continue
        }
    ...
    console.log('Done! All madeirafriends assets have been downloaded and replaced.')
  } catch (error) {
